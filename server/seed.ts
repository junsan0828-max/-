...
async function seed() {
  ...
  if (!db) {
    console.error("DB 연결 실패. .env 파일의 DB 정보를 확인하세요.");
    process.exit(1);
  }
  ...
  // ─── 트레이너 계정 1 ──────────────────────────────────────────────────────
  const existingTrainer1 = await db
    .select({ id: users.id })
    .from(users)
  ...
  if (!existingTrainer1[0]) {
    ...
    trainer1Id = (trainerInsert as any).insertId;
  ...
  if (trainer1Id) {
    ...
    if (existingMembers.length === 0) {
      ...
      // 회원 3: 활성, 정상
      const [m3] = await db.insert(members).values({
        trainerId: trainer1Id,
        name: "박민준",
        phone: "010-5555-6666",
        gender: "male",
