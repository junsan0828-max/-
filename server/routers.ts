...
import { z } from "zod";
import { eq, and, desc, sql, lte, gte, gt, like } from "drizzle-orm";
...
// 짐+ 세션용 별도 context (gymPlusMemberId를 req.session에 저장)
...
const adminOnly = t.procedure.use(({ ctx, next }) => {
...
const gymPlusRouter = t.router({
  ...
  listWorkoutLogs: gymPlusProtected
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input?.month) {
        logs = logs.filter(l => l.logDate.startsWith(input.month!));
      }
      return logs;
    }),
  ...
  createWorkoutLog: gymPlusProtected
    ...
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const [row] = await db.insert(gymPlusWorkoutLogs).values({
        gymPlusMemberId: ctx.gymPlusMemberId, ...input,
      }).returning();
      return row;
    }),
  ...
  updateWorkoutLog: gymPlusProtected
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!existing[0] || existing[0].gymPlusMemberId !== ctx.gymPlusMemberId)
        throw new TRPCError({ code: "FORBIDDEN" });
      const [row] = await db.update(gymPlusWorkoutLogs)
        .set({ ...data, updatedAt: new Date().toISOString() })
        .where(eq(gymPlusWorkoutLogs.id, id)).returning();
      return row;
    }),
  ...
  admin_updateVideo: adminOnly
    ...
    .mutation(async ({ input }) => {
      ...
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const { id, ...data } = input;
      await db.update(gymPlusVideos).set({ ...data, updatedAt: new Date().toISOString() }).where(eq(gymPlusVideos.id, id));
      return { success: true };
    }),

  admin_deleteVideo: adminOnly
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  // ── 어드민: 카테고리 관리 ──────────────────────────────────────────────────
  admin_listCategories: adminOnly.query(async () => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    return db.select().from(gymPlusVideoCategories).orderBy(gymPlusVideoCategories.sortOrder);
  }),

  admin_createCategory: adminOnly
  ...
  admin_deleteCategory: adminOnly
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  admin_updateEvent: adminOnly
    ...
    .mutation(async ({ input }) => {
      ...
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const { id, ...data } = input;
      await db.update(gymPlusEvents).set({ ...data, updatedAt: new Date().toISOString() }).where(eq(gymPlusEvents.id, id));
      return { success: true };
    }),

  admin_deleteEvent: adminOnly
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
...
const accessRouter = t.router({
  checkIn: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!found) {
        await db.insert(accessLogs).values({ phone: phoneForLog, accessResult: "not_found" });
        return { result: "not_found", member: null, locker: null };
      }

      const today = new Date().toISOString().substring(0, 10);
      const hasGymMembership = !!found.membershipEnd && found.membershipEnd >= today;
      const activePT = await db
        .select()
        .from(ptPackages)
        .where(and(eq(ptPackages.memberId, found.id), eq(ptPackages.status, "active")));
      const validPT = activePT.filter((p: any) => !p.expiryDate || p.expiryDate >= today);
      ...
      return {
        ...
        member: {
          ...
          membershipType,
          ptPackage: validPT.length > 0 ? {
            name: validPT[0].packageName,
            expiryDate: validPT[0].expiryDate,
            remainingSessions: (validPT[0].totalSessions ?? 0) - (validPT[0].usedSessions ?? 0),
          } : null,
        },
        locker: lockerRow[0]
          ? { lockerNumber: lockerRow[0].lockerNumber, type: lockerRow[0].lockerType, endDate: lockerRow[0].endDate }
          : null,
      };
    }),
  ...
  todayStats: protectedProcedure.query(async () => {
    ...
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    const today = new Date().toISOString().substring(0, 10);
    const logs = await db.select().from(accessLogs).where(like(accessLogs.accessedAt, `${today}%`));
    return {
      total: logs.length,
  ...
  getAccessLogs: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(accessLogs).where(where).orderBy(desc(accessLogs.accessedAt)).limit(input.limit).offset(input.date ? 0 : (input.page - 1) * input.limit);
    }),
  ...
  createLocker: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const [locker] = await db.insert(lockers).values({
  ...
  releaseLocker: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const [updated] = await db.update(lockers).set({
        memberId: null, memberName: null, memberPhone: null,
        isOccupied: 0, startDate: null, endDate: null,
        updatedAt: new Date().toISOString(),
      }).where(eq(lockers.id, input.lockerId)).returning();
      return updated;
    }),
  ...
  getMembersForLocker: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    return db.select({ id: members.id, name: members.name, phone: members.phone })
      .from(members)
