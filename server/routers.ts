...
친근하고 동기부여가 되는 톤으로 작성해주세요.`,
  ...
  admin_reviewMission: adminOnlyGymPlus
    .input(z.object({ submissionId: z.number(), status: z.enum(["approved", "rejected"]) }))
    .mutation(async ({ ctx, input }) => {
      const now = new Date().toISOString();
      await pool.query(
        `UPDATE gym_plus_mission_submissions SET status = $1, "reviewedAt" = $2, "reviewedBy" = $3 WHERE id = $4`,
        [input.status, now, ctx.user.id, input.submissionId]
      );
      return { success: true };
    }),
...
const kioskRouter = t.router({
  ...
  dietCheckIn: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const memberRes = await pool.query(
        `SELECT id, name, phone, username, "memberId", "programName", "programStartDate", "isActive"
         FROM gym_plus_members
         WHERE REGEXP_REPLACE(COALESCE(NULLIF(phone, ''), username, ''), '[^0-9]', '', 'g') = $1
         ORDER BY ("programName" IS NOT NULL) DESC, "isActive" DESC, id DESC
         LIMIT 1`,
      ...
      if (openSession.rows[0]) {
        ...
        return {
          ...
          elapsedMin: 0,
          message: workoutType === "cardio"
            ? `유산소 운동을 시작합니다! 운동이 끝나면 다시 체크인해 주세요. (최소 30분 이상)`
            : `영상 운동을 시작합니다! 수업이 끝나면 다시 체크인해 주세요. (최소 30분 이상)`,
        };
      }
    }),
