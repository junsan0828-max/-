  ...
  return (
    <div className={`relative bg-[#0d1117] rounded-[2rem] border border-white/10 overflow-hidden shadow-2xl shadow-black/60 ${className}`}>
      <div className="bg-[#080c12] px-5 pt-4 pb-2 flex items-center justify-between border-b border-white/5">
        <span className="text-[12px] font-semibold tracking-tight text-white">FIT <span className="text-blue-400">STEP</span></span>
    ...
    <PhoneFrame>
      <div className="p-4 space-y-3 bg-[#0a0f18]">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[["32", "총 회원", "text-white"], ["8", "오늘 수업", "text-blue-400"], ["94%", "재등록률", "text-green-400"]].map(([v, l, c]) => (
            <div key={l as string} className="bg-white/[0.04] rounded-xl p-2.5 text-center border border-white/5">
              <div className={`text-base font-semibold ${c as string}`}>{v as string}</div>
              <div className="text-[10px] text-white/35 mt-0.5">{l as string}</div>
            </div>
          ))}
        </div>
        {/* 출석 목록 */}
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5 space-y-2">
          <div className="text-[10px] font-semibold text-white/40 mb-2">오늘 수업 현황</div>
          {[["김민준", "10:00", true], ["이서연", "11:30", true], ["박지훈", "14:00", false], ["최유진", "16:00", false]].map(([n, t, done]) => (
            <div key={n as string} className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-500/30 to-violet-500/20 flex items-center justify-center text-[10px] font-semibold text-white/70 border border-white/10">{(n as string)[0]}</div>
              <div className="flex-1">
                <div className="text-[10px] text-white/70">{n as string}</div>
                <div className="text-[10px] text-white/30">{t as string}</div>
              </div>
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${done ? "bg-blue-500/15 text-blue-400" : "bg-white/5 text-white/25"}`}>
                {done ? "출석" : "예정"}
              </span>
            </div>
          ))}
        ...
        <div className="bg-gradient-to-r from-orange-500/15 to-red-500/8 border border-orange-500/20 rounded-xl p-3">
          <div className="text-[10px] font-semibold text-orange-400 mb-1.5">⚡ 재등록 임박 회원</div>
          {[["최유진", "3회 남음"], ["김태호", "1회 남음"]].map(([n, r]) => (
            <div key={n as string} className="flex justify-between items-center py-0.5">
              ...
              <span className="text-[10px] text-orange-400/80">{r as string}</span>
            </div>
          ))}
        </div>
        {/* 미니 차트 */}
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 mb-2">이번 달 출석률</div>
    ...
    <PhoneFrame>
      <div className="p-4 space-y-3 bg-[#0a0f18]">
        <div className="flex items-center justify-between mb-1">
          <div>
            <div className="text-xs font-semibold text-white/80">수업 전 컨디션 체크</div>
            <div className="text-[10px] text-white/30">김민준 · 2026.05.26</div>
          </div>
          <div className="text-[10px] px-2 py-1 bg-blue-500/20 text-blue-400 rounded-full border border-blue-500/30 font-semibold">출석</div>
        </div>
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 mb-2 font-semibold">컨디션 평가</div>
          <div className="flex gap-1.5">
            {[1,2,3,4,5].map(v => (
              <div key={v} className={`flex-1 py-2 text-center text-[10px] rounded-lg border font-semibold ${v === 4 ? "bg-blue-500/20 border-blue-500/40 text-blue-400" : "border-white/8 text-white/20"}`}>{v}</div>
            ))}
          </div>
        </div>
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 mb-2 font-semibold">수면시간</div>
        ...
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 mb-2 font-semibold">통증 부위</div>
        ...
        <div className="bg-white/[0.03] rounded-xl p-2.5 border border-blue-500/15 flex items-center gap-2">
          ...
          <div className="flex-1">
            <div className="text-[10px] text-white/60">PT 세션 1회 차감</div>
          </div>
          <div className="text-[10px] font-semibold text-blue-400">ON</div>
        </div>
      </div>
    </PhoneFrame>
  );
...
function ContractMockup() {
  return (
    <PhoneFrame>
      <div className="p-4 space-y-3 bg-[#0a0f18]">
        <div className="text-xs font-semibold text-white/80 mb-3">📄 전자계약서</div>
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5 space-y-2">
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3">
          <div className="text-[10px] text-blue-400/80 font-semibold mb-2">상담 메모</div>
          <div className="text-[10px] text-white/40 leading-relaxed">체중 감량 목표 -8kg. 다이어트 식단 병행. 주 3회 수업 희망. 허리 디스크 주의사항 있음.</div>
        </div>
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 mb-1.5 font-semibold">재등록 예정</div>
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1.5 rounded-full bg-white/8 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-violet-500" style={{ width: "78%" }} />
            </div>
            <span className="text-[10px] text-white/50">28/36회</span>
          </div>
        </div>
        <div className="flex gap-2">
          <div className="flex-1 py-2 text-center text-[10px] font-semibold bg-blue-500/20 border border-blue-500/30 text-blue-400 rounded-xl">서명 완료</div>
          <div className="flex-1 py-2 text-center text-[10px] text-white/30 border border-white/8 rounded-xl">재등록 안내</div>
        </div>
      </div>
    </PhoneFrame>
  );
...
function AnalyticsMockup() {
  return (
    <PhoneFrame>
      <div className="p-4 space-y-3 bg-[#0a0f18]">
        <div className="text-xs font-semibold text-white/80 mb-1">📊 성장 분석</div>
        <div className="grid grid-cols-2 gap-2">
          {[["재등록률", "94%", "↑ 3%", "green"], ["유지율", "87%", "↑ 5%", "blue"]].map(([l, v, d, c]) => (
            <div key={l as string} className={`bg-white/[0.03] border border-white/5 rounded-xl p-3`}>
              <div className="text-[10px] text-white/35 mb-1">{l as string}</div>
              <div className={`text-xl font-bold ${c === "green" ? "text-green-400" : "text-blue-400"}`}>{v as string}</div>
              <div className={`text-[10px] mt-0.5 ${c === "green" ? "text-green-400/60" : "text-blue-400/60"}`}>{d as string}</div>
            </div>
          ))}
        </div>
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 mb-2 font-semibold">월별 출석률 추이</div>
        ...
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5 space-y-2">
          <div className="text-[10px] text-white/35 font-semibold">출석 패턴</div>
          {[["월·수·금", 85], ["화·목", 72], ["주말", 45]].map(([d, p]) => (
            <div key={d as string} className="flex items-center gap-2">
              <span className="text-[10px] text-white/50 w-14 shrink-0">{d as string}</span>
              <div className="flex-1 h-1.5 rounded-full bg-white/8 overflow-hidden">
                <div className="h-full rounded-full bg-blue-500/60" style={{ width: `${p}%` }} />
              </div>
              <span className="text-[10px] text-white/35">{p}%</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <PhoneFrame>
      <div className="bg-[#0a0f18] p-4 space-y-3">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500/40 to-violet-500/30 border border-blue-500/30 flex items-center justify-center text-xs font-semibold text-blue-300">K</div>
          <div>
            <div className="text-xs font-semibold text-white/80">김민준님의 관리 페이지</div>
            <div className="text-[10px] text-blue-400">FITSTEP+ 회원</div>
          </div>
        </div>
        {/* 이번 달 운동 */}
        <div className="bg-gradient-to-r from-blue-600/15 to-violet-600/8 border border-blue-500/20 rounded-xl p-3">
          <div className="text-[10px] text-blue-400 font-semibold mb-2">이번 주 운동 프로그램</div>
          {[["월", "스쿼트 4×12 / 레그프레스", true], ["수", "벤치프레스 4×10 / 숄더프레스", true], ["금", "데드리프트 3×8 / 루마니안", false]].map(([d, w, done]) => (
            <div key={d as string} className="flex items-center gap-2 py-1">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-semibold ${done ? "bg-blue-500/30 text-blue-400" : "bg-white/5 text-white/25"}`}>{done ? "✓" : d as string}</div>
              <span className="text-[10px] text-white/50 flex-1">{w as string}</span>
            </div>
          ))}
        </div>
        {/* 영상 */}
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 font-semibold mb-2">추천 운동 영상</div>
          {[["스쿼트 자세 교정", "3:42"], ["데드리프트 기초", "5:21"]].map(([title, dur]) => (
            <div key={title as string} className="flex items-center gap-2 py-1.5">
              ...
              <span className="text-[10px] text-white/55 flex-1">{title as string}</span>
        ...
        {/* 식단 */}
        <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
          <div className="text-[10px] text-white/35 font-semibold mb-2">오늘 식단</div>
          <div className="grid grid-cols-3 gap-1.5">
            {[["단백질", "165g", "blue"], ["탄수화물", "220g", "amber"], ["지방", "55g", "violet"]].map(([n, v, c]) => (
              <div key={n as string} className="text-center">
                <div className={`text-[10px] font-semibold ${c === "blue" ? "text-blue-400" : c === "amber" ? "text-amber-400" : "text-violet-400"}`}>{v as string}</div>
...
function SectionLabel({ text }: { text: string }) {
  return <p className="text-center text-[12px] font-semibold tracking-[0.2em] text-blue-400/60 uppercase mb-4">{text}</p>;
}
    ...
    <div className="min-h-screen bg-[#060910] text-white overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#060910]/96 backdrop-blur-xl border-b border-white/5" : ""}`}>
        <div className="max-w-5xl mx-auto px-5 py-4 flex items-center justify-between">
          <span className="text-lg font-bold tracking-tight select-none">FIT <span className="text-blue-400">STEP</span></span>
          <div className="flex items-center gap-2">
            <a href="/auth/kakao" className="text-sm text-white/45 hover:text-white px-3 py-1.5 transition-colors">로그인</a>
            <a href="/auth/kakao" className="text-sm font-semibold px-4 py-1.5 rounded-full bg-blue-500 hover:bg-blue-400 transition-all shadow-lg shadow-blue-500/25">무료 시작</a>
          </div>
        </div>
      </nav>
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="text-center lg:text-left order-2 lg:order-1">
              <div className="inline-flex items-center gap-2 text-[12px] font-semibold tracking-widest text-blue-400 bg-blue-400/10 border border-blue-400/20 rounded-full px-4 py-1.5 mb-8">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse shrink-0" />
                피트니스 성장 플랫폼
              </div>
              <h1 className="text-[2.5rem] sm:text-[3rem] font-bold leading-[1.1] tracking-tight mb-6">
                STEPER의 성장과<br />운영을 연결하다.
              </h1>
              <p className="text-[16px] text-white/42 leading-loose mb-10">
                회원관리부터 상담, 건강리포트, 개인 브랜딩까지.<br />
                STEPER와 강사를 위한 피트니스 성장 플랫폼.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start mb-8">
                <a href="/auth/kakao"
                  className="px-7 py-3.5 rounded-xl bg-blue-500 hover:bg-blue-400 transition-all font-semibold text-[16px] shadow-xl shadow-blue-500/25 hover:-translate-y-0.5 hover:shadow-blue-500/40 text-center">
                  무료로 시작하기
                </a>
                <a href="/auth/kakao"
                  className="px-7 py-3.5 rounded-xl border border-white/10 hover:border-white/22 bg-white/[0.03] hover:bg-white/5 transition-all font-medium text-[16px] text-white/55 hover:text-white hover:-translate-y-0.5 text-center">
                  로그인
                </a>
              </div>
            ...
            <div className="relative order-1 lg:order-2 flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[300px]">
                ...
                <div className="absolute -left-8 top-1/3 z-20 w-44 bg-[#111827] border border-blue-500/20 rounded-2xl p-3 shadow-xl shadow-black/40">
                  <div className="text-[10px] font-semibold text-blue-400 mb-2">✓ 출석 체크</div>
                  <div className="text-[12px] font-semibold text-white/80">김민준</div>
                  <div className="text-[10px] text-white/35 mb-2">컨디션 4/5 · 수면 7h</div>
                ...
                <div className="absolute -right-6 bottom-16 z-20 w-40 bg-[#111827] border border-orange-500/25 rounded-2xl p-3 shadow-xl shadow-black/40">
                  <div className="text-[10px] font-semibold text-orange-400 mb-1.5">⚡ 재등록 임박</div>
      ...
      <section className="py-24 px-5 bg-[#070b12]">
        <div className="max-w-3xl mx-auto">
          ...
          <h2 className="text-center text-[2rem] sm:text-[2.25rem] font-bold leading-tight tracking-tight mb-4">왜 FIT STEP이<br />필요한가</h2>
      ...
      <section className="py-24 px-5">
        <div className="max-w-5xl mx-auto">
          ...
          <h2 className="text-center text-[2rem] sm:text-[2.25rem] font-bold leading-tight tracking-tight mb-4">현장에서 필요한<br />기능만 담았습니다</h2>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
              <div className={`rounded-2xl border ${COLOR_MAP.blue.border} bg-gradient-to-br ${COLOR_MAP.blue.bg} p-7`}>
                ...
                <h3 className="text-xl font-bold mb-3 leading-snug">회원 한 명 한 명을<br />체계적으로 관리하세요</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className={`rounded-2xl border ${COLOR_MAP.violet.border} bg-gradient-to-br ${COLOR_MAP.violet.bg} p-6`}>
                ...
                <h3 className="text-base font-bold mb-2 leading-snug">상담부터 재등록까지</h3>
              ...
              <div className={`rounded-2xl border ${COLOR_MAP.cyan.border} bg-gradient-to-br ${COLOR_MAP.cyan.bg} p-6`}>
                ...
                <h3 className="text-base font-bold mb-2 leading-snug">데이터로 성장 방향을</h3>
      ...
      <section className="py-24 px-5 bg-[#070b12] relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="flex justify-center mb-6">
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-gradient-to-r from-blue-600/25 to-violet-600/15 border border-blue-500/30 text-xs font-semibold text-blue-300 tracking-widest uppercase">
              <span>✦</span> FIT STEP+
            </div>
          </div>
      ...
      <section className="py-24 px-5">
        <div className="max-w-4xl mx-auto">
          ...
          <h2 className="text-center text-[2rem] sm:text-[2.25rem] font-bold leading-tight tracking-tight mb-4">현장을 아는 사람이<br />만든 플랫폼</h2>
            ...
            <div className="space-y-3">
              {CULTURES.map(c => (
                <div key={c.n} className="flex gap-5 rounded-2xl border border-white/6 bg-white/[0.025] hover:border-blue-500/22 hover:bg-blue-500/[0.03] transition-all duration-300 p-5">
                  <span className="text-3xl font-bold shrink-0 leading-none" style={{ background: "linear-gradient(180deg, rgba(96,165,250,0.5) 0%, rgba(96,165,250,0.1) 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{c.n}</span>
      ...
      <section className="py-28 px-5 bg-[#070b12] relative overflow-hidden">
        ...
        <div className="relative max-w-xl mx-auto text-center">
          <div className="text-[12px] font-bold tracking-widest text-blue-400/60 uppercase mb-8">지금 시작하세요</div>
          ...
          <a href="/auth/kakao"
            className="inline-flex items-center gap-2.5 px-10 py-4 rounded-xl bg-blue-500 hover:bg-blue-400 transition-all font-semibold text-base shadow-2xl shadow-blue-500/30 hover:shadow-blue-500/50 hover:-translate-y-0.5">
            무료로 시작하기
      ...
      <footer className="border-t border-white/5 py-10 px-5">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="font-semibold text-base tracking-tight">FIT <span className="text-blue-400">STEP</span></span>
