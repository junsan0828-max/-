...
export default function LeadsPage() {
  ...
  const createMutation = trpc.leads.create.useMutation({
    onSuccess: () => { toast.success("상담이 등록되었습니다"); utils.leads.invalidate(); resetForm(); },
    onError: (e) => toast.error(e.message),
  });
  const updateMutation = trpc.leads.update.useMutation({
    onSuccess: () => { toast.success("수정되었습니다"); utils.leads.invalidate(); resetForm(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const statCounts = STATUS_OPTIONS.map(s => ({
    ...s, count: (leadsData ?? []).filter(r => r.lead.status === s.value).length,
  }));
            ...
            <div className="overflow-y-auto flex-1 p-4 space-y-4">
              ...
              <div>
                ...
                {form.interestType === "PT" && (
                  <div className="mt-2">
                    <label className="text-xs text-muted-foreground">운동 목적</label>
                    <div className="flex flex-wrap gap-2 mt-1">
