    ...
    <div className="space-y-4 pb-8">
      ...
      >
        <div>
          ...
          <p className="text-[16px] font-semibold text-white">새 시퀀스 작성</p>
          <p className="text-[12px] text-white/65 mt-0.5">시퀀스 메이커로 바로 시작</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => setLocation("/sequences/library")}
          ...
          <p className="text-sm font-semibold">시퀀스 라이브러리</p>
          <p className="text-[12px] text-muted-foreground mt-0.5">승인된 시퀀스 둘러보기</p>
        </button>
        <button onClick={() => setLocation("/sequences/mine")}
          ...
          {!!unread?.count && (
            <span className="absolute top-3 right-3 min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-[10px] font-semibold flex items-center justify-center">{unread.count}</span>
          )}
          <ListChecks className="h-5 w-5 text-violet-500 mb-2" />
          <p className="text-sm font-semibold">내 시퀀스</p>
          <p className="text-[12px] text-muted-foreground mt-0.5">작성·검토·보관 현황</p>
        </button>
      </div>
      ...
      <button onClick={() => setLocation("/sequences/mine")} className="w-full rounded-2xl bg-card border border-border p-4 flex items-center justify-between hover:border-primary/40 transition-colors">
        ...
        <span className="text-base font-semibold">{credits?.balance ?? 0}개</span>
      </button>

      <div className="rounded-2xl bg-accent/30 border border-border/60 p-4 space-y-1.5">
        <p className="text-xs font-semibold text-foreground/70">시퀀스 랩은 이렇게 진행돼요</p>
