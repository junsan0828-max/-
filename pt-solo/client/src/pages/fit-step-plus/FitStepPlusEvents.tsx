...
export default function FitStepPlusEvents({ trainerId }: { trainerId: number }) {
  const [, navigate] = useLocation();
  const base = `/fit-step-plus/${trainerId}`;
  const [filter, setFilter] = useState<string | undefined>();

  const { data: events, isLoading } = trpc.fitStepPlus.listEvents.useQuery(
    filter ? { trainerId, eventType: filter } : { trainerId }
  );
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {e.startDate && e.endDate && (
                    <p className="text-[10px] text-muted-foreground mt-1">{e.startDate} ~ {e.endDate}</p>
                  )}
