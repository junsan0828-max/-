...
export default function FitStepPlusEventDetail({ eventId, trainerId }: { eventId: number; trainerId: number }) {
  const [, navigate] = useLocation();
  const base = `/fit-step-plus/${trainerId}`;
  const { data: event, isLoading } = trpc.fitStepPlus.getEvent.useQuery({ id: eventId });
  ...
  if (!event) {
    return (
      <div className="p-4 text-center">
        ...
        <Button variant="ghost" size="sm" onClick={() => navigate(`${base}/events`)}>목록으로</Button>
      </div>
    );
  }
